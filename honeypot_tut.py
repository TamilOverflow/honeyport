...
def scan_visitor(visiter_ip_address):
    ...
    with open(file_log_path+file_log_name,is_write_or_append) as fp:
        get_port_details = get_port_information(visiter_ip_address)
...
def get_port_information(ip_address):
    ...
    print("[+] Scanning Visiter Inprogress..")
    sc = {}
    for host in scanner.all_hosts():
        ...
        for proto in scanner[host].all_protocols():
            ...
            for port in lport:
                a = "port:" + str(port) + "  Service Name:"+sc[port]['name'] + "  Product Name:"+ sc[port]['product'] +" Version Name:"+ sc[port]['version'] +" Status:"+ sc[port]['state']
